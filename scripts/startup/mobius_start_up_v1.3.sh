...
sed -i "s/ExecStart=.*/ExecStart=\/usr\/local\/bin\/hostkeyd start -c https:\/\/comet-hn1.exogeni.net:8111\/ -s $WORKFLOWID -r $RTOKEN -w $WTOKEN -f $FAMILY -k dynamo-broker1.exogeni.net:9093 -t mobius-promeithus/g" /usr/lib/systemd/system/host-key-tools.service

systemctl enable host-key-tools.service  

cd /root/ 

#wget  http://geni-images.renci.org/images/kthare10/certs/ca-cert -O /var/private/ssl/ca.crt
#wget  http://geni-images.renci.org/images/kthare10/certs/kafkacat-ca1-signed.pem -O /var/private/ssl/client.pem
#wget  http://geni-images.renci.org/images/kthare10/certs/kafkacat.client.key -O /var/private/ssl/key.pem
